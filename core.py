...
def calcular_aporte(aporte_mensal, poupanca_inicial, meses_acc, taxa_mensal, cota_bruta, matriz_cotas_liq, saque_mensal):
...
def bissecao(tipo_objetivo, outro_valor, poupanca_inicial, meses_acc, taxa, cota_bruta, matriz_cotas_liq, saque_mensal, precisao=0.01, max_iter=100):
    if tipo_objetivo == "zerar":
        ...
        if outro_valor is None or outro_valor < 0:
            raise ValueError("É necessário informar o valor final desejado válido.")
    ...
    if melhor_aporte is None:
        raise ValueError("🟥 Os dados informados exigem um valor muito alto ou uma idade de vida muito longa. Tente ajustar a expectativa de vida, renda desejada ou rentabilidade para valores mais realistas.")
...
def simular(dados):
    for k, v in dados.items():
        if isinstance(v, (int, float)) and v < 0:
            raise ValueError("⚠️ Por favor, preencha todos os valores com números positivos ou zero.")
    ...
    patrimonio, patrimonio_final = calcular_aporte(
        aporte_ideal,
        dados["poupanca_atual"],
        meses_acc,
        taxa,
    ...
    if patrimonio_final < 0:
        raise ValueError("🟥 O patrimônio final ficou negativo. Ajuste os dados para um cenário mais viável.")

    return round(aporte_ideal, 2), patrimonio.tolist()
