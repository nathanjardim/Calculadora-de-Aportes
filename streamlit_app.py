...
with st.form("form_inputs"):
    ...
    poupanca_atual = st.number_input("Poupana atual (R$)", min_value=0, value=1_000_000, step=1000)

    st.markdown("###  Dados Econmicos")
    ...
    st.markdown("###  Renda")
    previdencia = st.number_input("Previdncia (R$)", min_value=0, value=0, step=100)
    outras_rendas = st.number_input("Aluguel ou outras fontes (R$)", min_value=0, value=0, step=100)

    st.markdown("###  Fim do Patrimnio")
...
if submitted:
    ...
    if isinstance(aporte, str):
        ...
        st.metric("Poupana necessria", f"R$ {poupanca_necessaria:,.2f}")
        st.metric("Percentual da renda atual", f"{percentual_renda * 100:.2f}%")

        st.markdown("###  Evoluo do Patrimnio")
        ...
        df_export = pd.DataFrame({
            ...
            "Patrimnio": df_chart["Montante"]
        })

        def gerar_excel():
            output = BytesIO()
            with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                df_export.to_excel(writer, index=False, sheet_name="Simulao")
            output.seek(0)
            return output
